#!/usr/bin/env node
// One-time importer for runbook Task C4 (docs/plans/2026-09-02-operations-and-data-runbook.md):
// loads data/socio-legal-journals.quickstatements.txt (QuickStatements v1 syntax,
// TAB-separated) and creates/updates the corresponding items on live Wikidata via the
// Wikibase REST API v1. This is a fork of ops-import-associations.mjs pointed at the
// journals file and its own QID map instead of the associations one.
//
// Usage:
//   node scripts/ops-import-journals.mjs --dry-run   # parse + print, no writes
//   node scripts/ops-import-journals.mjs             # parse + write for real
//
// Credentials: WIKIDATA_BOT_USERNAME / WIKIDATA_BOT_PASSWORD from .env (gitignored).
// Output: data/journal-qids.json (journal label -> QID mapping) plus a full log to stdout.

import { readFileSync, existsSync, writeFileSync } from 'node:fs';

const IMPORT_FILE = 'data/socio-legal-journals.quickstatements.txt';
const QIDS_OUT = 'data/journal-qids.json';
